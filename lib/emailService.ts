...
class EmailService {
  ...
  async sendEmail({ to, subject, html }: EmailOptions): Promise<boolean> {
    try {
      const mailOptions = {
        // Use env var or fallback, don't hardcode your personal email in shared code
        from: process.env.EMAIL_FROM || '"Support Team" <noreply@yourapp.com>', 
        to,
        subject,
        html,
      };
  ...
  // ... (Keep your existing generatePasswordResetEmail function here) ...
