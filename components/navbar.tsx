    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      {variant === "default" && mobileMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-black/95 backdrop-blur-xl border-b border-white/10 shadow-2xl md:hidden flex flex-col p-4 gap-4 animate-in slide-in-from-top-5 fade-in duration-200">
            <Link href="/explore" onClick={() => setMobileMenuOpen(false)} className="w-full">
              <Button variant="ghost" className="text-white hover:text-purple-400 cursor-pointer text-lg w-full justify-start">
                Explore
              </Button>
            </Link>
            ...
            {isAuthenticated ? (
              ...
              <div className="flex flex-col gap-3 w-full">
                <Link href="/login" onClick={() => setMobileMenuOpen(false)} className="w-full">
                  <Button variant="outline" className="cursor-pointer text-lg w-full">
                    Login
                  </Button>
                </Link>
